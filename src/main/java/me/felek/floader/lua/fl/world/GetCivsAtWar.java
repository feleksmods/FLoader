...
public class GetCivsAtWar extends TwoArgFunction {
    @Override
    public LuaValue call(LuaValue arg, LuaValue luaValue1) {
        ...
        if (luaValue1.checkint() < 0 || luaValue1.checkint() >= CFG.core.getCivsSize()) {
            ExitCode.WORLD_CIV_INVALID_ID.throwFatalError("CivID " + luaValue1.checkint() + " out of [0-" + (CFG.core.getCivsSize()-1) + "]");
        }
        if (CFG.core.getCiv(arg.checkint()) == null) ExitCode.WORLD_CIV_NULL.throwFatalError("Civ object at " + arg.checkint() + " is null");

        return LuaValue.valueOf(CFG.core.getCivsAtWar(arg.checkint(), luaValue1.checkint()));
    }
}
