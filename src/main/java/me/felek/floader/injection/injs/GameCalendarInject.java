...
public class GameCalendarInject implements Injection {
    ...
    public void inject(ClassPool pool, String clname) throws NotFoundException, CannotCompileException {
        ...
        mNextDays.insertBefore("{ me.felek.floader.api.event.EventBus.call(\"onDayChange\", ($w)$1); }");
        ...
        mNextDays.instrument(new ExprEditor() {
            public void edit(javassist.expr.FieldAccess f) throws javassist.CannotCompileException {
                if (f.getFieldName().equals("currMonth") && f.isWriter()) {
                    f.replace("{ $proceed($$); me.felek.floader.api.event.EventBus.call(\"onMonthChange\", ($w)age.of.civilizations2.jakowski.lukasz.GameCalendar.currMonth); }");
                }
            }
        });
    }
